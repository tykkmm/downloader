...
def download_video(message, video):
    ...
    if "videoplayernew" in link and ("brightcove" in link or len(link.split("/")[-1]) == 13):
        vid_id = link[-13:]
        link = f"https://videoplayernew.herokuapp.com/{vid_id}"
    elif "videoplayernew" in link and "jwp" in link:
        vid_id = link[-8:]
        link = f"https://videoplayernew.herokuapp.com/{vid_id}"
    ...
    if "youtu" in link:
        if vid_format in ["144", "240", "480"]:
            ...
            ytf = 18
    elif ("videoplayernew" in link and len(link.split("/")[-1]) == 13):
        vid_id = link[-13:]
        link = f"http://videoplayernew.herokuapp.com/{vid_id}"
        if vid_format not in ["144", "240", "360", "480", "720"]:
            vid_format = "360"
        ytf = f"'bestvideo[height<={vid_format}]+bestaudio'"
    elif ("videoplayernew" in link and len(link.split("/")[-1]) == 8):
        if vid_format == "144":
            vid_format = "180"
        elif vid_format == "240":
            vid_format = "270"
